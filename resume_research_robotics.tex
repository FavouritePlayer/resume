...
\begin{document}
    ...
    \section{Projects}
        ...
        \begin{onecolentry}
            \begin{highlights}
                ...
                \item Terrain adaptation: policy trained on randomized heightfields \textbf{achieves 2.1x the reward} (893 vs. 424) and 40\% lower fall rate than the flat-trained baseline on unseen rough terrain.
                \item Leg-damage robustness: a specialist policy trained with one leg permanently amputated reaches $\sim$49x the flat-baseline reward on that leg, but \textbf{does not transfer to other legs} (100\% fall rate).
                \item Built a compound policy router that switches checkpoints by damaged leg, \textbf{beating any single policy} (836 vs. 497 mean reward) across a 4-leg $\times$ 10-seed evaluation sweep.
    ...
    \section{Skills}
        ...
        \begin{onecolentry}
            \textbf{Technologies:} PyTorch, MuJoCo, Gymnasium, Stable-Baselines3, NumPy, dlib, scipy, Git
        \end{onecolentry}
